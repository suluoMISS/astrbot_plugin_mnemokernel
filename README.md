...
上传完整 v0.1.1 ZIP 时，插件还会在启动时校验并从包内 wheel 自动修复缺失的 `native_runtime`。
仅复制 HTML/Python 文件时，页面会提示升级完整安装包，不会绕过内核直接读取 SQLite。
旧 AstrBot 不支持页面接口时，原有聊天命令继续工作。

升级旧版本时无需删除数据库：v0.1.0 使用的 Schema 4 数据会由 v0.1.1 原生内核直接打开并保留。
如果 `/mnemo doctor` 显示 Schema 4 和三项能力均为 `true`，但页面仍提示浏览不可用，说明当前进程加载的原生模块缺少浏览接口；重载插件或完全重启后，插件会优先加载包内并自动修复的 v0.1.1 runtime。
